...
export class DataService {
  ...
  async ensureCatalogLoaded(force = false): Promise<void> {
    ...
    this.catalogRequest = Promise
      .all([
        firstValueFrom(this.http.get<ApiCollectionResponse<Category>>(`${environment.apiBaseUrl}/categories`)),
        firstValueFrom(this.http.get<ApiCollectionResponse<Product>>(`${environment.apiBaseUrl}/products`))
      ])
